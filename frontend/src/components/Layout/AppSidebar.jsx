...
import {
  ...
  Sparkles, ChevronRight, Camera, UserRound, Mail, GraduationCap,
} from 'lucide-react';
    ...
    <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-slate-950 text-white shadow-2xl">
      ...
      <div className="px-5 py-5">
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <input
            id="profile-picture-modal-input"
            type="file"
          ...
          />
          <label
            htmlFor="profile-picture-modal-input"
            className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-2xl bg-indigo-500 px-4 py-3 text-sm font-black text-white transition hover:bg-indigo-600"
          >
          ...
          <button
            type="button"
            onClick={onRemove}
            disabled={saving || !user?.profile_picture_url}
            className="rounded-2xl bg-white/[0.08] px-4 py-3 text-sm font-black text-slate-200 transition hover:bg-white/[0.12] disabled:cursor-not-allowed disabled:opacity-40"
          >
            Remove photo
          </button>
        </div>
...
const AppSidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 flex h-dvh w-[min(18rem,86vw)] flex-shrink-0 flex-col border-r border-white/10
        ...
        transition-transform duration-300 ease-in-out lg:sticky lg:top-0 lg:z-auto lg:h-screen lg:w-72 lg:translate-x-0
      ...
      `}>
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 lg:hidden">
          ...
          <button onClick={onClose} className="rounded-xl p-1.5 text-slate-400 transition-all hover:bg-white/10 hover:text-white" aria-label="Close menu">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="shrink-0 px-3 pt-4 pb-3 sm:px-4">
          <button
            type="button"
            onClick={() => { setProfileOpen(true); setProfileMsg(''); }}
            className="w-full rounded-3xl border border-white/10 bg-white/[0.06] p-3 text-left shadow-2xl shadow-indigo-950/30 outline-none backdrop-blur transition hover:border-indigo-300/30 hover:bg-white/[0.08] focus:ring-2 focus:ring-indigo-400/50"
          >
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 break-all text-[11px] leading-4 text-slate-300 sm:text-xs">{user?.email}</p>
              </div>
            ...
            <p className="mt-3 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wide text-indigo-200/80">
              <Camera className="h-3 w-3" /> Click to update profile
            </p>
          </button>
          ...
          {!user?.verified && (
            <div className="mt-3 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-2">
              ...
              <button
                ...
                className="w-full rounded-xl bg-amber-400/15 px-3 py-2 text-[11px] font-black text-amber-200 transition-colors hover:bg-amber-400/25 disabled:opacity-50"
              >
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-3 sm:px-4 [scrollbar-width:thin] [scrollbar-color:rgba(148,163,184,0.35)_transparent]">
            ...
            {NAV_ITEMS.map(({ to, icon: Icon, label, exact }) => {
              ...
              return (
                <Link
                  ...
                  to={to}
                  onClick={onClose}
                  className={`group flex items-center justify-between rounded-2xl px-3 py-2.5 text-sm font-bold transition-all sm:py-3 ${
                    active
                ...
                >
                  <span className="flex min-w-0 items-center gap-3">
                    <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${active ? 'bg-white/15' : 'bg-white/[0.04] group-hover:bg-white/10'}`}>
          ...
          <div className="mt-5">
            ...
            <Link
              to="/mfa-setup"
              onClick={onClose}
              className={`flex items-center justify-between rounded-2xl px-3 py-2.5 text-sm font-bold transition-all sm:py-3 ${
            ...
            >
              <div className="flex min-w-0 items-center gap-3">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-white/[0.05]"><Shield className="h-4 w-4" /></span>
        ...
        <div className="shrink-0 space-y-1 border-t border-white/10 bg-slate-950/70 p-3 sm:p-4">
          <button
            onClick={toggleTheme}
            className="flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-bold text-slate-400 transition-all hover:bg-white/[0.07] hover:text-white"
          >
            <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-white/[0.04]">{isDark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</span>
            {isDark ? 'Light Mode' : 'Dark Mode'}
          </button>
          <button
            onClick={handleLogout}
            className="flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-bold text-slate-400 transition-all hover:bg-red-500/10 hover:text-red-300"
          >
            <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-white/[0.04]"><LogOut className="h-4 w-4" /></span>
            Log Out
          </button>
        </div>
      </aside>

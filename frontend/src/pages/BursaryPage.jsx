...
import {
  Search, ExternalLink, BookOpen, Award, Clock, CheckCircle2, Building, Cpu, HeartPulse,
  Briefcase, Scale, FlaskConical, HardHat, GraduationCap, Landmark, Globe, ChevronDown, ChevronUp,
  Target, Bookmark, ArrowUpDown, X,
} from 'lucide-react';
import BURSARIES from '../data/bursaries.json';
import { Alert, Badge, Button, EmptyState, Input, PageHeader, Select } from '../components/ui';
...
const daysUntil = (dateString) => Math.ceil((new Date(dateString) - new Date()) / 864e5);

const formatDeadline = (dateString) => new Date(dateString).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' });
  ...
  return (
    <article className={cn('rounded-2xl border border-slate-200 bg-white p-5 shadow-card transition-all duration-200 hover:border-brand-200 hover:shadow-card-hover dark:border-slate-800 dark:bg-slate-900 dark:hover:border-brand-900', expired && 'opacity-75')}>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        <Badge tone={FUNDER_TONE[bursary.funder] || 'neutral'} size="sm" className="capitalize">{bursary.funder}</Badge>
        {bursary.level.map((level) => <Badge key={level} size="sm" className="capitalize">{level}</Badge>)}
      </div>

      <p className="mt-3 text-sm font-semibold text-emerald-700 dark:text-emerald-400">{bursary.amount}</p>
      <p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300">{bursary.description}</p>
      ...
      >
        ...
        {expanded ? 'Hide requirements' : `View requirements (${bursary.requirements.length})`}
      </button>

      {expanded && (
        <ul className="mt-3 flex flex-wrap gap-1.5 animate-fade-in">
          {bursary.requirements.map((requirement) => (
      ...
      <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button href={bursary.applicationUrl} target="_blank" rel="noopener noreferrer" size="sm" variant={expired ? 'secondary' : 'primary'}>
...
const BursaryPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return BURSARIES
      .filter((bursary) => {
        if (filters.field !== 'all' && bursary.field !== filters.field) return false;
        if (filters.funder !== 'all' && bursary.funder !== filters.funder) return false;
        if (filters.level !== 'all' && !bursary.level.includes(filters.level)) return false;
        if (query && ![bursary.title, bursary.provider, bursary.field].some((value) => value.toLowerCase().includes(query))) return false;
        return true;
      })
      .sort((a, b) => new Date(a.deadline) - new Date(b.deadline));
  }, [filters]);

  const openCount = filtered.filter((bursary) => daysUntil(bursary.deadline) > 0).length;
  const urgentCount = filtered.filter((bursary) => { const days = daysUntil(bursary.deadline); return days > 0 && days <= 30; }).length;
  const hasActiveFilters = filters.field !== 'all' || filters.funder !== 'all' || filters.level !== 'all' || Boolean(filters.search);
  const activeFieldLabel = FIELDS.find((item) => item.value === filters.field)?.label;
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-4 sm:px-6 lg:px-8 lg:py-6">
      <PageHeader
        ...
        description="Find bursaries today, with internships, graduate roles, and competitions coming to the hub soon."
        className="mb-4"
      />

      <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {OPPORTUNITY_TYPES.map(({ title, text, icon: Icon, active }) => (
          <div key={title} className={cn('rounded-2xl border p-4 shadow-card', active ? 'border-brand-200 bg-brand-50 dark:border-brand-900 dark:bg-brand-500/10' : 'border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900')}>
            <div className="flex items-start justify-between gap-3">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-brand-600 shadow-sm dark:bg-slate-900 dark:text-brand-300">
                <Icon className="h-5 w-5" aria-hidden="true" />
              </span>
              {active ? <Badge tone="success" size="sm">Live</Badge> : <Badge size="sm">Coming soon</Badge>}
            </div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          { value: urgentCount, label: 'Closing within 30 days' },
      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-base font-bold text-slate-950 dark:text-white sm:text-lg">
            {filters.field !== 'all' ? activeFieldLabel : 'All bursaries'} <span className="font-medium text-slate-400">({filtered.length})</span>
          </h2>
      ...
      {filtered.length > 0 ? (
        <div className="space-y-3">
          {filtered.map((bursary) => <BursaryCard key={bursary.id} bursary={bursary} />)}
        </div>
      ) : (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-card dark:border-slate-800 dark:bg-slate-900">
          <EmptyState
            ...
            description="Try a different field of study or clear your filters."
            action={<Button onClick={() => setFilters(INITIAL_FILTERS)}>Clear filters</Button>}
          />
        </div>
      )}

...
const STATUS_META = {
  ...
  under_review: {
    label: 'Under review',
    className: 'border-brand-200 bg-brand-50 text-brand-700 dark:border-brand-900 dark:bg-brand-500/10 dark:text-brand-300',
  },
  approved: {
    label: 'Approved',
...
const StatusBadge = ({ status }) => {
  ...
  return <span className={`inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-bold ${meta.className}`}>{meta.label}</span>;
};
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6">
        <div className="mb-6 rounded-3xl bg-slate-950 p-6 text-white shadow-sm dark:bg-slate-900 dark:ring-1 dark:ring-slate-800">
          <p className="mb-2 inline-flex rounded-full bg-brand-500/15 px-3 py-1 text-xs font-bold text-brand-200 ring-1 ring-brand-400/20">My Application</p>
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          <button onClick={onAccommodation} className="group overflow-hidden rounded-3xl border border-brand-200 bg-white p-6 text-left shadow-sm transition-all hover:-translate-y-1 hover:border-brand-400 hover:shadow-xl dark:border-brand-900/60 dark:bg-slate-900">
            <div className="flex items-start justify-between gap-3">
              ...
              {checkingApplication ? <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-500 dark:bg-slate-800 dark:text-slate-300">Checking...</span> : hasApplication && <StatusBadge status={existingApplication.status} />}
            </div>
            ...
            {hasApplication && (
              <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-950/60">
                ...
                <p className="mt-1 font-mono text-sm font-bold text-slate-950 dark:text-white">{existingApplication.reference || existingApplication.id}</p>
              </div>
            )}
          ...
          <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="absolute right-4 top-4 inline-flex items-center gap-1 rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">
            ...
            <button disabled className="mt-6 inline-flex cursor-not-allowed items-center gap-2 rounded-xl bg-slate-100 px-5 py-2.5 text-sm font-bold text-slate-400 dark:bg-slate-800 dark:text-slate-500">Coming soon</button>
          </div>
        </div>
      </div>
    </div>
...
const StudentApplicationPage = () => {
  ...
  const renderStep = () => {
    ...
    return <>{stepHeader('Review and submit', 'Confirm your details before submitting your accommodation application.', ShieldCheck)}<div className="space-y-4 text-sm"><div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800"><p className="font-bold text-slate-950 dark:text-white">Applicant</p><p className="mt-1 text-slate-500 dark:text-slate-400">{form.firstName} {form.lastName} · {form.email} · {form.phoneNumber}</p></div><div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800"><p className="font-bold text-slate-950 dark:text-white">Studies</p><p className="mt-1 text-slate-500 dark:text-slate-400">{form.university?.toUpperCase()} · {form.studentNumber} · {form.faculty} · {form.yearOfStudy}</p></div><div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800"><p className="font-bold text-slate-950 dark:text-white">Selected residences</p><p className="mt-1 text-slate-500 dark:text-slate-400">{selectedPropertyNames.join(', ') || 'None selected'}</p></div><label className="flex gap-3 rounded-2xl border border-brand-100 bg-brand-50 p-4 dark:border-brand-900 dark:bg-brand-500/10"><input type="checkbox" checked={termsAccepted} onChange={(e) => setTermsAccepted(e.target.checked)} /><span className="text-sm text-slate-700 dark:text-slate-300">I confirm that the information provided is accurate and I agree to the <Link to="/terms" className="font-bold text-brand-600 underline">Terms</Link> and <Link to="/privacy" className="font-bold text-brand-600 underline">Privacy Policy</Link>.</span></label>{errors.terms && <p className="text-xs font-bold text-red-600">{errors.terms}</p>}{submitError && <p className="rounded-xl bg-red-50 p-3 text-xs font-bold text-red-700 dark:bg-red-500/10 dark:text-red-300">Something went wrong while submitting. Please try again.</p>}</div></>;
  };
  ...
  if (existingApplication) {
    ...
    return <div className="min-h-screen bg-slate-50 dark:bg-slate-950"><div className="mx-auto max-w-4xl px-4 py-6"><button onClick={() => navigate('/application')} className="mb-4 inline-flex items-center rounded-xl bg-white px-3 py-2 text-xs font-bold text-brand-600 shadow-sm dark:bg-slate-900 dark:text-brand-300"><ArrowLeft className="mr-1 h-3.5 w-3.5" />Application types</button><div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10"><CheckCircle className="h-6 w-6" /></div><h1 className="text-2xl font-bold text-slate-950 dark:text-white">Accommodation application submitted</h1><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Your application is saved and ready for review by the relevant property team.</p><div className={`mt-5 inline-flex rounded-full border px-3 py-1 text-xs font-bold capitalize ${meta.className}`}>{meta.label}</div><div className="mt-6 rounded-2xl border border-slate-200 p-4 dark:border-slate-800"><p className="text-xs font-bold text-slate-400">Reference</p><p className="mt-1 font-mono text-sm font-bold text-slate-950 dark:text-white">{existingApplication.reference || existingApplication.id}</p></div></div></div></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <div className="mx-auto w-full max-w-7xl px-3 py-4 sm:px-4 lg:px-6">
        ...
        <div className="mb-5 rounded-3xl bg-slate-950 p-6 text-white shadow-sm dark:bg-slate-900 dark:ring-1 dark:ring-slate-800"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-bold sm:text-3xl">Accommodation application</h1><p className="mt-2 text-sm text-slate-300">Browse sections freely. Required fields are checked only on final submit.</p></div><div className="inline-flex w-fit items-center gap-2 rounded-2xl bg-white/10 px-4 py-2 text-sm font-bold text-white"><MapPin className="h-4 w-4" /> Step {step} of 6</div></div></div>
        <div className="mb-5 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="mb-4 flex items-center justify-between"><p className="text-sm font-bold text-slate-950 dark:text-white">Accommodation</p><p className="text-xs font-bold text-brand-600">{Math.round((step / STEPS.length) * 100)}% viewed</p></div><div className="mb-5 h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div className="h-full rounded-full bg-brand-600" style={{ width: `${(step / STEPS.length) * 100}%` }} /></div><div className="grid grid-cols-2 gap-2 md:grid-cols-6">{STEPS.map(({ id, title, icon: Icon }) => <button key={id} onClick={() => goToStep(id)} className={`rounded-2xl border px-3 py-3 text-xs font-bold transition-all ${step === id ? 'border-brand-500 bg-brand-50 text-brand-700 dark:bg-brand-500/10 dark:text-brand-300' : 'border-slate-200 text-slate-500 hover:border-brand-300 dark:border-slate-800 dark:text-slate-400'}`}><Icon className="mx-auto mb-1 h-4 w-4" />{title}</button>)}</div></div>
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">{renderStep()}<div className="mt-8 flex flex-col gap-3 border-t border-slate-100 pt-5 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between"><button type="button" onClick={() => goToStep(Math.max(1, step - 1))} disabled={step === 1} className="inline-flex items-center justify-center rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-40 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"><ArrowLeft className="mr-2 h-4 w-4" />Previous</button>{step < 6 ? <button type="button" onClick={() => goToStep(step + 1)} className="inline-flex items-center justify-center rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-brand-700">Next<ArrowRight className="ml-2 h-4 w-4" /></button> : <button type="button" onClick={submit} disabled={loading} className="inline-flex items-center justify-center rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-brand-700 disabled:opacity-50">{loading ? 'Submitting...' : 'Submit application'}<CheckCircle className="ml-2 h-4 w-4" /></button>}</div></div>
      </div>
    </div>
  );
};
